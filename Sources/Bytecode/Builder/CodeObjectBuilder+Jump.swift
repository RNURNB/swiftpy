...
extension CodeObjectBuilder {
  ...
  public func appendJumpAbsolute(to label: NotAssignedLabel) throws {
    let arg = try self.appendExtendedArgsForLabelIndex(label)
  ...
  public func appendPopJumpIfTrue(to label: NotAssignedLabel) throws {
    let arg = try self.appendExtendedArgsForLabelIndex(label)
    self.append(.popJumpIfTrue(labelIndex: arg))
  }

  /// Append a `popJumpIfFalse` instruction to this code object.
  public func appendPopJumpIfFalse(to label: NotAssignedLabel) throws {
    let arg = try self.appendExtendedArgsForLabelIndex(label)
  ...
  public func appendJumpIfTrueOrPop(to label: NotAssignedLabel) throws {
    let arg = try self.appendExtendedArgsForLabelIndex(label)
    self.append(.jumpIfTrueOrPop(labelIndex: arg))
  }

  /// Append a `jumpIfFalseOrPop` instruction to this code object.
  public func appendJumpIfFalseOrPop(to label: NotAssignedLabel) throws {
    let arg = try self.appendExtendedArgsForLabelIndex(label)
